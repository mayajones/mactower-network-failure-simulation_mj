...
# 2013 Dec 20, use recently updated DIP yeast PPI data
rm(list=ls())
...
table(matches)
#FALSE  TRUE 
#3932   1050  #this is amazingly consistent with the Taiwan group's report.

matches = uniq.orf.from.pairs %in% unique(essenTb$orf[essenTb$essenflag=='nonessential'])
table(matches)
# FALSE  TRUE 
# 1453  3520  #this is amazingly consistent with Taiwan group's report.

#remove unmatched orfs from PPI
...
# FALSE  TRUE 
# 7562  3006
#So, intxn among essential genes appear to be suppressed. 
...
# Notice that degree*p > 1 if the cells are to be alive. 
p_v = seq(0.3, 1.0, by=0.1)  ; #the chance that each gene interaction is active at t=0
...
for(lambda in lambda_v) {
  ...
  write.csv(sim, file="scePPIaging_sim_20131220_tmp.csv", row.names=F)
...
write.csv(sim, file="scePPIaging_sim_2013120_end.csv", row.names=F)
