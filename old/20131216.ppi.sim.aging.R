...
lambda_v = 1 / 3^seq(2,3)
#lambda_v = 1 / s

#p_v = seq(0.1, 1.0, by=0.1)  ; #the chance that each gene interaction is active at t=0
p_v = c(0.7,  0.8, 0.9, 1.0)  ; #the chance that each gene interaction is active at t=0
...
for(lambda in lambda_v) {
  ...
  write.csv(sim, file="scePPIaging_sim_20131217_tmp.csv", row.names=F)
...
write.csv(sim, file="scePPIaging_sim_20131217_end.csv", row.names=F)
