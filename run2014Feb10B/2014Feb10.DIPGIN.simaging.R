...
#set.seed(999999), #2014Jan20 change
...
p_v = c(0.6, 0.6,  0.7, 0.7,  0.8, 0.8, 0.9, 0.9, 1.0, 1.0)  ; #the chance that each gene interaction is active at t=0
...
for(lambda in lambda_v) {
  # p=0.9, #for debug
  for( p in p_v) {  
    popSize = 1000 #too small pop size and too small p can lead to very few living individuals
...
write.csv(sim, file="sceGINPPIaging_sim_2014Feb10_end.csv", row.names=F)
